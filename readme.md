...
[14. 이미지 업로드 & 이미지 서버 만들기](#이미지-업로드-이미지-서버-만들기)
...
- join operation 을 통해 다른 테이블(콜렉션) 참조해서 가져올 수 있는 데이터를 구태여 하나의 테이블(콜렉션)에 다 때려박는 관습 (지키길 권고함)
...
- MongoDB 에 데이터 입출력 하는 nodejs 라이브러리 2개 있음
  - MongoDB native driver
  - Mongoose
    - schema 를 정의하기 때문에 data validation 에 유용
- Mongoose 안쓰고 MongoDB Compass 라는 프로그램 or MongoDB shell (CLI) 등을 써서 data validation 할 수도 있음. 물론 MongoDB native driver 에서 js 로 if 문 등 써서 할 수도 있고.
...
- 악성 유저라면 ?
  ...
  - 버튼 두번 누르거나 해서 중복된 요청?
  - 회원가입시 아이디란에 한글
  - 필수 입력칸인데 빈칸
  - xss
    - 아이디 등 input 입력란에 <scipt></script>  혹은 db 에서 정보 꺼내는 문법 작성
  - brute force
    - 유저 비밀번호 100만번 시도해서 때려 맞추는 경우
    - 해결 방법 예시 1)  express-rate-limit 등의 라이브러리 이용

3 . 관리자 기능?

- DB 에 회원정보 저장할 때 role : '관리자' 등의 필드 집어 넣기.

4 . 앞으로 해야 할 것들

- 보안 강화 기술
- 사용자가 업로드한 이미지, 파일 다루는 방법( 리사이즈, 축소 ,검열 등)
- OAuth 소셜 로그인
